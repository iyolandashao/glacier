...
%initial
[x2d y2d] = ndgrid(x,y);
H         =  0.5*exp( -(x2d-0.5*Lx).^2/0.1 - (y2d-0.5*Ly).^2/0.1);
surf(x2d,y2d,H),shading flat,light,view(123,32),title('H')
B         = 0.3*x2d + cos(x2d); mesh(x2d,y2d,B),title('B(x,y)')
surf(x2d,y2d,B),shading flat,light,view(123,32),title('B')
surf(x2d,y2d,H+B),shading flat,light,view(123,32),title('S')
%action
for it=1:nt
Hx     = (H(1:end-1,:)+H(2:end,:))/2;
Hy     = (H(:,1:end-1)+H(:,2:end))/2;
qx     = -g/(3*mu_f)* Hx.^3.*diff(H,1,1)/dx;
qy     = -g/(3*mu_f)* Hy.^3.*diff(H,1,2)/dy;
